...
public class CookieUtil {
    ...
    public static boolean isLogin(HttpServletRequest req){
        ...
        return !StringUtils.isEmpty(ticket) && req.getSession().getAttribute(ticket) != null;
    }
    ...
    /*用户登录到session*/
    public static void login(HttpServletRequest req, HttpServletResponse res, User user){
        String ticket = UUID.randomUUID().toString();
        HttpSession session = req.getSession();
        session.setAttribute(ticket,user);
        CookieUtil.setCookie(req,res,"userTicket",ticket,3600,false);
    }
